...
class Controller_Admin_Categories extends Controller_Admin {
    ...
    public function action_add()
    {
        ...
        if (isset($_POST['submit']))
        {
            ...
            $data = Arr::extract($_POST, array('parent_id', 'title', 'url', 'priority', 'content', 'title_menu', 'keywords', 'description', 'create_date', 'status'));
    ...
    public function action_edit()
    {
        ...
        if (isset($_POST['submit'])) {
            $_POST['status'] = (isset($_POST['status'])) ? 1 : 0;
            $data = Arr::extract($_POST, array('parent_id', 'title', 'url', 'priority', 'content', 'title_menu', 'keywords', 'description', 'status'));
            $data['full_url'] = $category->make_full_url($data['parent_id'], $data['url']);
            $category->values($data);
    ...
    // Сохранение текущей сортировки категорий.
    public function action_sort_save()
    {
        $pages = $_POST['pages'];
        
        for($i = 0; $i < count($pages); $i++) {
            $id = (int)$pages[$i];
            $category = ORM::factory('Category', $id)->set('priority', $i)->update();
        }
        echo 'Success!';
        die();
    }
